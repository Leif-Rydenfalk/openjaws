// cdk.rs - The Sovereign Narrative Substrate for the Rheo Living Mesh (Rust Edition)
...
impl RheoCell {
    ...
    pub fn merge_atlas(&self, incoming: HashMap<String, AtlasEntry>) {
        ...
        if let Ok(addr) = self.addr.read() {
            if !addr.is_empty() {
                atlas.insert(self.id.clone(), AtlasEntry {
                    addr: addr.clone(),
                    caps: self.handlers.read().unwrap().keys().cloned().collect(),
                    pub_key: Some(self.pub_key_hex.clone()),
                    last_seen: now,
                    last_gossiped: now,
                    gossip_hop_count: 0,
                });
    ...
    pub async fn listen(self: Arc<Self>) {
        ...
        println!("🟢 [Rust] {} Online @ {}", self.id, port);
